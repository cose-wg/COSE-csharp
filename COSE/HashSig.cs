...
namespace Com.AugustCellars.COSE
{
    public class HashSig
    {
        ...
        class LmsKey
        {
            private LmsAlgorithmType lmsType;
            private LmotsAlgorithmType lmotsType;
...
BuildFromSeed(seed);
            ...
            {
                lmsType = (LmsAlgorithmType) Int32.Parse(input[offset]);
                lmotsType = (LmotsAlgorithmType) Int32.Parse(input[offset + 1]);
                byte[] seed = StringToByteArray(input[offset + 2]);
            ...
            private void BuildFromSeed(byte[] seed)
            {
                ...
                int keyCount = 1 << this.H;
                int p = Constants.Values[(int) this.lmotsType].p;

                this.x = new byte[keyCount][];
                //  Build LMS Private Key
                for (int q = 0; q < keyCount; q++) {
                    byte[] qBytes = u32str(q);
                    //  Build LM-OTS private key for each leaf
                    this.x[q] = new byte[this.M*p];
            ...
            private int M
            {
                get {
                    switch (lmsType) {
                        ...
                        default:
                            throw new CoseException($"Unknown LmsAlgorithmType value {lmsType}");
                    }
                }
            }
            ...
            static byte[] D_INTR = new byte[2]{0x83, 0x83};
            static byte[] D_LEAF = new byte[2]{0x82, 0x82};
            private static byte[] D_PBLC = new byte[2] {0x80, 0x80};

            private byte[] ComputePubPart(int i, int two2H)
            {
                Sha256Digest digest = new Sha256Digest();
                digest.BlockUpdate(this.Identifier, 0, this.Identifier.Length);
            ...
            {
                byte[] key = new byte[8 + this.Identifier.Length + this.M];
                Array.Copy(u32str((Int32) lmsType), key, 4);
                Array.Copy(u32str((Int32) lmotsType), 0, key, 4, 4);
                Array.Copy(this.Identifier, 0, key, 8, this.Identifier.Length);
                Array.Copy(ComputePubPart(1, (int) Math.Pow(2, this.H)), 0, key, 8+this.Identifier.Length, 32);
            ...
            {
                int p = Constants.Values[(int) lmotsType].p;
                int limit = (1 << Constants.Values[(int) lmotsType].w) - 1;
                ushort i;
                byte[] q = u32str(node);
                byte[] jX = new byte[1];
                byte[][] y = new byte[p][];
                for (i = 0; i < p; i++) {
                    ...
                    Array.Copy(x[node], i*32, tmp, 0, 32);
                    byte[] iX = u16str(i);
                ...
                Array.Copy(u32str((int) lmotsType), pubKey, 4);
            ...
            {
                Constants c = Constants.Values[(int) lmotsType];
                int n =  c.HashLength;
                int p = c.p;
                int w = c.w;
                byte[] C = new byte[n];
                ...
                byte[][] y = new byte[p][];
                byte[] Q = ComputeHash(new Sha256Digest(),
                                       new byte[][] {this.Identifier, u32str(this.LeafNumber), D_MESG, C, message});

                byte[] signature = new byte[4 + C.Length + p * this.M];
                Array.Copy(u32str((Int32) this.lmotsType), 0, signature, 0, 4);
                ...
                for (int i = 0; i < p; i++) {
                    ...
                    Array.Copy(tmp, 0, signature, 4 + C.Length + this.M * i, tmp.Length);
                }
        ...
        public HashSig(string privateKey)
        {
            ...
            HssTree = new HssNode[levels];
            for (int i = 0; i < levels; i++) {
                HssTree[i] = new HssNode(input, i*5+1);
            }
        }       

        public void Create(LmsAlgorithmType[] lmsType, LmotsAlgorithmType lmotsType)
        {
            HssTree = new HssNode[lmsType.Length];
            for (int i = 0; i < lmsType.Length; i++) {
                HssTree[i] = new HssNode(lmsType[i], lmotsType);
            }
        }
        ...
        {
            get {
                byte[] rootKey = HssTree[0].PublicKey;
                byte[] pubKey = new byte[rootKey.Length + 4];
                Array.Copy(u32str(HssTree.Length), pubKey, 4);
